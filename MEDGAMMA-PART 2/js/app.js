...
function generateAINotes() {
    ...
    setTimeout(() => {
        ...
        const notes = generateSOAPNotes(patient);

        document.getElementById('noteSubjective').textContent = notes.subjective;
        document.getElementById('noteObjective').textContent = notes.objective;
        document.getElementById('noteAssessment').textContent = notes.assessment;
        document.getElementById('notePlan').textContent = notes.plan;
...
function generateSOAPNotes(patient) {
    const v = patient.vitals;
    const isAbnormal = patient.riskLevel === 'high' || patient.riskLevel === 'medium';

    return {
        subjective: `Patient ${patient.name} (${patient.age}${patient.gender}) presents for ${patient.condition} monitoring. ${patient.notes}. Patient ${isAbnormal ? 'reports mild discomfort' : 'denies any acute complaints'}.`,
        ...
        assessment: `${patient.age}${patient.gender} with ${patient.condition} condition. ${isAbnormal ? 'Current presentation concerning for possible exacerbation. Elevated vitals require monitoring and possible intervention.' : 'Condition stable. Vitals and presentation reassuring.'}`,

        plan: isAbnormal
            ? '1. Continue telemetry monitoring\n2. Repeat vitals in 30 minutes\n3. Consider ECG if HR remains elevated\n4. Notify attending if condition worsens\n5. Review current medications'
            : '1. Continue routine monitoring\n2. Maintain current treatment plan\n3. Reassess in 4 hours\n4. Encourage ambulation as tolerated'
    };
}
